...
class RincianangkakreditController extends Controller
{
    ...
    public function edit($id_rinci_ak)
    {
        $rincianangkakredit = DB::table('master_rincian_angka_kredit')->where('id_rinci_ak',$id_rinci_ak)        
        ->join('master_unsur_utama', 'master_rincian_angka_kredit.id_unsur_utama', '=', 'master_unsur_utama.id')
        ->join('master_subunsurs', 'master_rincian_angka_kredit.id_subunsur', '=', 'master_subunsurs.id_sub_unsur')            
        ->join('master_rincian_kegiatan', 'master_rincian_angka_kredit.id_rincian_kegiatan', '=', 'master_rincian_kegiatan.id_rincian_kegiatan')  
        ->join('master_tingkatan_wi', 'master_rincian_angka_kredit.id_tingkatan_wi', 'master_tingkatan_wi.id_tingkatan_wi' )
        ->select('master_rincian_angka_kredit.*','master_unsur_utama.unsur_utama', 'master_subunsurs.kegiatan_sub_unsur', 'master_rincian_kegiatan.rincian_kegiatan', 'master_rincian_kegiatan.satuan', 'master_tingkatan_wi.nama_tingkatan') 
        ->first();
        //dd($rincianangkakredit);
        return view('rincianangkakredit.edit', compact('rincianangkakredit'));
    }

    public function update(Request $request, $id_rinci_ak)
    {
        $rincianangkakredit = \App\Rincianangkakredit::find($id_rinci_ak);
        if($rincianangkakredit) {
            $rincianangkakredit->angka_kredit = $request->angka_kredit;
            $rincianangkakredit->save();
        }
        return redirect()->route('rincianangkakredit.index')->with('success', 'Hasil Rincianangkakredit udpdated successfully');
    }
}
